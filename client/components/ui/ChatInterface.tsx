...
import { MessageSquare, X, Volume2, VolumeX, Mic, MicOff, Loader2 } from 'lucide-react';
...
export default function ChatInterface({ 
  ...
  const [agentId, setAgentId] = useState<string | undefined>(elevenlabsAgentId || undefined); // Use provided agent ID or undefined
  const [connectionInProgress, setConnectionInProgress] = useState<boolean>(false);
  ...
  const [micTranscriptionCount, setMicTranscriptionCount] = useState<number>(0);
  const [transcriptionCheckTimer, setTranscriptionCheckTimer] = useState<NodeJS.Timeout | null>(null);
  ...
  const conversation = useConversation({
    ...
    onDisconnect: () => {
      ...
      if (activeConversation && !isMuted) {
        console.log("IMPORTANT: Immediate reconnection to prevent audio cutoff");
        ...
        if (message.isFinal) {
          ...
          if (message.transcription.trim().length > 0) {
            ...
            const updatedHistory = [
              ...conversationHistory,
              { role: 'user', content: message.transcription.trim() }
            ];
            setConversationHistory(updatedHistory);
            
            // Send transcript to the API
            handleAPIRequest(message.transcription.trim(), updatedHistory);
          }
        }
      } 
    ...
    onError: (error) => {
      ...
      if (errorString.includes('agent') && errorString.includes('does not exist')) {
        setAgentError(true);
      }
    },
    volume: isMuted ? 0 : 1,
    ...
    containerElement: typeof document !== 'undefined' ? 
      document.getElementById('elevenlabs-audio-container') || undefined : 
      undefined
  });
  ...
  const handleAPIRequest = async (text: string, history: Array<{role: string, content: string}>) => {
    try {
      ...
      if (updatedHistory.length === 0 || updatedHistory[0].role !== 'system') {
        updatedHistory = [
          { role: 'system', content: `Document Context: ${contextString}` },
          ...updatedHistory
        ];
      } else {
      ...
      console.log("Sending document context with history:", updatedHistory[0]);
...
Use this information to provide accurate responses about the document. If the user asks about content not present in this document, you can mention that it's not covered in the current document or page.`;
      ...
      const response = await fetch('http://100.124.42.82:3005/api/generate/chat', {
        method: 'POST',
  ...
  const [elevenlabsConnected, setElevenlabsConnected] = useState<boolean>(false);
  ...
  const logSessionStatus = () => {
    console.log(`ElevenLabs Session Status: ${status}, Active Conversation: ${activeConversation}, Muted: ${isMuted}`);
  };

  // Add API key state
  const [apiKey, setApiKey] = useState<string | null>(elevenlabsApiKey || null);
  ...
  const directTTS = async (text: string) => {
    try {
      ...
      let audioContainer = document.getElementById('elevenlabs-direct-audio-container');
      if (!audioContainer) {
      ...
      // Then try localStorage as fallback
        ...
        // If still no key, prompt the user
          ...
          const promptedKey = prompt("Please enter your ElevenLabs API key to enable voice features:");
          if (promptedKey) {
            ...
            console.error("No API key provided - can't perform TTS");
            alert("ElevenLabs API key is required for voice features. Voice will not work.");
            return false;
          }
        }
      }
      ...
      console.log(`Sending TTS request to ${endpoint} for voice ID ${agentId}`);
      ...
      if (playPromise !== undefined) {
        playPromise.catch(error => {
          ...
          if (error.name === 'NotAllowedError') {
            console.log("⚠️ Audio autoplay blocked - creating interactive button");
  ...
  const speakText = async (text: string) => {
    ...
    if (!sdkSuccess && !isMuted) {
      console.log("SDK speech failed or not available, trying direct API...");
      directTTS(text);
    }
  };
  ...
  const trySpeaking = (text: string): boolean => {
    ...
    try {
      console.log("🔈 SPEAKING TEXT through ElevenLabs SDK:", text.substring(0, 50) + "...");
      console.log("🔈 Conversation object status:", status);
      console.log("🔈 Available methods:", Object.keys(conversation).join(', '));
      ...
      if (typeof (conversation as any).playText === 'function') {
        ...
        console.error("🔈 NO VALID SPEAKING METHOD FOUND ON CONVERSATION OBJECT");
  ...
  const setElevenLabsKey = () => {
    try {
      const currentKey = apiKey || localStorage.getItem('elevenlabs-api-key') || '';
      const newKey = prompt("Enter your ElevenLabs API key:", currentKey);
      
      if (newKey !== null) { // Not cancelled
        if (newKey.trim()) {
  ...
  const testDirectElevenLabsAPI = async () => {
    try {
      ...
      const testButton = document.getElementById('elevenlabs-test-direct');
      if (testButton) {
        testButton.textContent = "Testing...";
      }
      
      // Use the direct TTS function we created
      const success = await directTTS("This is a direct test of the ElevenLabs voice API using your API key.");
  ...
  const ensureElevenLabsConnection = async () => {
    if (!agentId) {
      console.warn("No ElevenLabs Agent ID provided. Voice features will not work.");
      return;
    }
    ...
    if (status !== 'connected' && !connectionInProgress) {
      try {
        setConnectionInProgress(true);
        console.log("⚠️ CONNECTING TO ELEVENLABS - ONE TIME CONNECTION ⚠️");
        console.log("⚠️ Using Agent ID:", voiceId);
        
        // Test if we can create an audio element first
        try {
          const testAudio = new Audio();
          console.log("📢 Audio element created successfully:", !!testAudio);
        } catch (e) {
        ...
        setTimeout(() => {
          if (!isMuted) {
            playTestSound();
            console.log("🔈 Testing ElevenLabs audio with message...");
  ...
  const toggleMicrophone = async (): Promise<void> => {
    ...
    if (!agentId) {
      alert("No ElevenLabs Agent ID configured. Voice features will not work.");
    ...
    try {
      ...
      if (status !== 'connected') {
        ...
        try {
          ...
          alert("Could not connect to voice service. Please try again.");
          setConnectionInProgress(false);
          return;
        }
  ...
  useEffect(() => {
    ...
    const micCheckInterval = setInterval(() => {
      ...
      if (micInputLevelRef.current > 30 && micTranscriptionCount === 0) {
        console.log("🔄 Microphone appears to be working but not transcribing - attempting to fix");
    ...
    const reconnectionTimeout = setTimeout(() => {
      if (isListening && micTranscriptionCount === 0 && !connectionInProgress) {
        console.log("⚠️ No transcriptions received after extended period - performing full reconnection");
        
        // Perform a full reconnection
        const fullReconnect = async () => {
          try {
            console.log("Attempting full ElevenLabs reconnection for microphone");
            ...
            console.log("✅ Completed full microphone reconnection");
            
            // Add helpful message
            addMessage('ai', "I've reset the microphone connection. Please try speaking again.");
          } catch (e) {
  ...
  }, [isListening, micTranscriptionCount, agentId, conversation, connectionInProgress]);
  ...
  useEffect(() => {
    if (activeConversation && status !== 'connected' && !isMuted && !connectionInProgress && agentId) {
  ...
  useEffect(() => {
    ...
    if (activeConversation && status === 'connecting') {
      ...
      console.log('Current state:', { 
        ...
        streamingMessage: streamingMessage ? 'Yes (length: ' + streamingMessage.length + ')' : 'No'
      });
    }
  ...
  useEffect(() => {
    ...
    audioContainerRef.current = document.getElementById('elevenlabs-audio-container') as HTMLDivElement;
  ...
  useEffect(() => {
    return () => {
      ...
      if (status === 'connected' && !isPlayingAudio) {
        try {
          console.log("Preparing ElevenLabs session for clean shutdown");
          // Don't end immediately
          setTimeout(() => {
            try {
              conversation.endSession();
            } catch (e) {
              console.log("Ignoring error during delayed session end:", e);
            }
          }, 500);
        } catch (e) {
        ...
        console.log("Not disconnecting ElevenLabs - audio might still be playing");
      }
    };
  }, [status, conversation, isPlayingAudio]);

  // Add a pulsing effect when the mic is active
  const MicrophonePulse = ({ isActive, transcriptionCount }: { isActive: boolean, transcriptionCount: number }) => {
    if (!isActive && transcriptionCount === 0) return null;
    
    return (
      <div className={`absolute -inset-1 rounded-full animate-pulse ${isActive ? 'bg-green-500 opacity-30' : 'bg-yellow-500 opacity-20'}`} />
    );
  };
    ...
    const visualizeMicInput = async () => {
      try {
        ...
        const checkMicLevel = () => {
          ...
          if (avg > 30) {
            console.log(`🎤 Microphone input level: ${avg.toFixed(1)}`);
          }
          
          // If we're hearing sound but not getting transcriptions
          if (avg > 40 && micTranscriptionCount === 0) {
            console.log("⚠️ WARNING: Detecting audio input but no transcriptions received");
          }
  ...
  useEffect(() => {
    ...
    if (documentId && currentPage !== undefined && pageContent) {
      console.log("Document context changed to:", documentId, "page:", currentPage);
      
      // If document ID has changed, we need a complete reset
      // For page changes, we update the context but preserve the dialogue ID
      const documentChanged = dialogueId && documentIdRef.current !== documentId;
      
      // Update our reference to the current document ID
      documentIdRef.current = documentId;
      
      if (documentChanged) {
        console.log("Document ID changed - performing full conversation reset");
...
Objects mentioned: ${pageContent.objects?.join(', ') || 'None'}`;
      ...
      setConversationHistory(history => {
        ...
        updatedHistory.push({ role: 'assistant', content: pageChangeMsg });
        
        return updatedHistory;
      });
    } else if (prevPageRef.current === undefined && currentPage !== undefined) {
  ...
  useEffect(() => {
    ...
    if (isChatOpen && !hasShownWelcomeMessage && conversationMessages.length === 0 && documentSummary && pageContent) {
      // Create a welcome message with document context
      const welcomeMsg = `Welcome! I'm your JFK Document Assistant. I'm currently looking at document ${documentId}, page ${currentPage || 1}.
...
How can I help you understand this document?`;
      ...
      setConversationHistory([
        { role: 'system', content: `Document Context for ${documentId}, page ${currentPage}: ${pageContent.summary}` },
  ...
  }, [isChatOpen, hasShownWelcomeMessage, conversationMessages.length, documentId, currentPage, documentSummary, pageContent, addMessage, agentId, isMuted]);
  ...
  const playTestSound = () => {
    try {
      ...
      audio.volume = 0.5;
      audio.play().then(() => {
        console.log("✅ Test sound playback initiated successfully");
      }).catch(e => {
          ...
          <div style={{
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: '14px', fontWeight: 'bold' }}>
                ...
                {isSpeaking && ' (Speaking...)'}
                {isListening && ' (Listening' + (micInputLevelRef.current > 20 ? ' 🎤 ' + '|'.repeat(Math.min(10, Math.floor(micInputLevelRef.current / 10))) : '...') + ')'}
                {connectionInProgress && ' (Connecting...)'}
              </h3>
              {/* Add document context info */}
              {pageContent && (
                <div style={{ fontSize: '12px', opacity: 0.9, marginTop: '2px' }}>
                  Page {currentPage} of Document {documentId.substring(0, 8)}...
                </div>
              )}
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button
                onClick={() => {
                  console.log("🔍 DEBUG: ElevenLabs Status:", status);
                  console.log("🔍 DEBUG: Connected:", elevenlabsConnected);
                  ...
                  if (!isMuted) {
                    ...
                    speakText("This is a test message to verify audio is working correctly.");
                  }
                }}
                style={{
                  background: 'none',
          ...
          >
            ...
            {conversationMessages.length === 0 && !streamingMessage && (
              <div style={{ 
                ...
                {pageContent ? 
                  `Ask me about this document${currentPage ? ` (page ${currentPage})` : ''} and I'll help you understand its contents and historical context.` : 
            ...
            {conversationMessages.map((msg, idx) => (
              <div
                ...
                style={{
                  marginBottom: '10px',
                  color: msg.sender === 'user' ? '#1d4ed8' : '#047857',
                  backgroundColor: msg.sender === 'user' ? '#eff6ff' : '#ecfdf5',
                  padding: '8px 12px',
                  borderRadius: '8px',
                  maxWidth: '85%',
            ...
            {agentError && (
              <div style={{ 
                ...
                <p>Error: The ElevenLabs Agent ID is invalid or does not exist.</p>
            ...
            {!agentId && !agentError && (
              <div style={{ 
                ...
                <p style={{ marginTop: '8px' }}>
                  Voice features will not work. Please create an agent in ElevenLabs at{' '}
                  <a href="https://elevenlabs.io/app/conversational-ai" target="_blank" rel="noopener noreferrer"
            ...
            {conversationMessages.length === 0 && !streamingMessage && pageContent && (
              <div style={{ 
                marginTop: '15px',
                display: 'flex',
                flexWrap: 'wrap',
                ...
                <div style={{ width: '100%', textAlign: 'center', color: '#6b7280', fontSize: '12px', marginBottom: '5px' }}>
                  Try asking:
                </div>
                <button
                  onClick={() => handleSendMessage(`Can you summarize this document for me?`)}
                  style={{
                    fontSize: '12px',
                    padding: '6px 10px',
                ...
                <button
                  onClick={() => handleSendMessage(`Who are the key people mentioned on this page?`)}
                  style={{
                    fontSize: '12px',
                    padding: '6px 10px',
                    backgroundColor: '#f3f4f6',
                ...
                <button
                  onClick={() => handleSendMessage(`What is the historical significance of this document?`)}
                  style={{
                    fontSize: '12px',
                    padding: '6px 10px',
                    backgroundColor: '#f3f4f6',
                ...
                <button
                  onClick={() => handleSendMessage(`Can you explain the context of "${pageContent.places && pageContent.places.length > 0 ? pageContent.places[0] : pageContent.names && pageContent.names.length > 0 ? pageContent.names[0] : 'this document'}"?`)}
                  style={{
                    fontSize: '12px',
                    padding: '6px 10px',
                    backgroundColor: '#f3f4f6',
          ...
          <div style={{
            ...
            <input
              ...
              style={{
                ...
                marginRight: '8px',
                fontSize: '13px',
                backgroundColor: isListening ? '#f0fdf4' : 
                                connectionInProgress ? '#fef3c7' : 'white'
              }}
            ...
            <button
              ...
              style={{
                backgroundColor: (!userInput.trim() || connectionInProgress) ? '#e5e7eb' : '#3b82f6',
                color: (!userInput.trim() || connectionInProgress) ? '#9ca3af' : 'white',
                border: 'none',
                borderRadius: '4px',
                padding: '8px 12px',
                fontSize: '13px',
                cursor: (!userInput.trim() || connectionInProgress) ? 'not-allowed' : 'pointer'
              }}
            >
              Send
            </button>
