...
export async function GET(request: Request) {
	try {
		...
		if (!prisma.document) {
			console.error(
				'[DOCUMENT STATUS API] Prisma client not properly initialized'
			);
		...
		const documentId =
			searchParams.get('documentId') || searchParams.get('id');
		const groupsParam = searchParams.get('groups');

		// Parse document groups for filtering
		const enabledGroups = groupsParam
			? groupsParam.split(',').map((g) => g.trim().toLowerCase())
			: null;
		...
		try {
			...
			console.error(
				'[DOCUMENT STATUS API] Database connection failed:',
				dbError
			);
			return NextResponse.json(
				{
		...
		if (enabledGroups && enabledGroups.length > 0) {
			...
			where = {
				OR: [{ documentGroup: { in: enabledGroups } }],
			};
		...
		const transformedDocuments = documents.map((doc) => {
			...
			return {
				id: doc.id,
				status: docJson?.analysisComplete
					? 'ready'
					: 'waitingForAnalysis',
