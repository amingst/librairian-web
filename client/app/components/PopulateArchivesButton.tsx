...
export default function PopulateArchivesButton() {
          ...
          <DialogHeader>
            <DialogTitle>
              {isPopulating 
                ? 'Populating Archives...' 
                : result?.success 
                  ? 'Archives Populated Successfully' 
                  : 'Population Failed'}
            </DialogTitle>
            <DialogDescription>
              {isPopulating 
                ? 'Fetching documents from archives.gov and adding them to the database. This may take a few minutes...'
                : result?.success
                  ? `Added ${result.newDocuments} new documents to the database.`
                  : 'There was an error populating the archives.'}
            </DialogDescription>
          </DialogHeader>
            ...
            <Alert variant="default" className="bg-green-50 border-green-200">
              ...
              <AlertDescription>
                <ul className="text-sm mt-2 space-y-1">
                  <li>Total documents from archives.gov: {result.totalDocuments}</li>
                  <li>Existing documents in database: {result.existingDocuments}</li>
                  <li>New documents added: {result.newDocuments}</li>
                </ul>
              </AlertDescription>
            </Alert>
          )}
