...
export default function JFKFilesPage() {
	...
	const [localDocumentIdMap, setLocalDocumentIdMap] =
		useState<Record<string, string>>(documentIdMap);
	...
	const [processingMode, setProcessingMode] = useState<
		'full' | 'images-only' | 'stop-at-image'
	>('full');
	...
	const handleProcessDocument = async (documentId: string) => {
		try {
			...
			setConfirmationMessage(
				`Are you sure you want to process document ${documentId}?`
			);
			setConfirmationAction(() => async () => {
				try {
					...
					const isRfkDocument =
						documentId.toLowerCase().includes('rfk') ||
						documents.some((doc) => {
							const docWithType = doc as JFKDocument & {
								documentType?: string;
								documentGroup?: string;
							...
							return (
								doc.id === documentId &&
								(docWithType.documentGroup === 'rfk' ||
									docWithType.documentType === 'rfk')
							);
						});
					...
					if (finalizeResponse.ok) {
						console.log(
							`Document ${documentId} successfully finalized (type: ${
								isRfkDocument ? 'rfk' : 'jfk'
							})`
						);
						...
						console.log(
							'Document processing complete - data should now be visible in the UI'
						);
					} else {
	...
	return (
		...
		>
			...
			>
				<Button
					...
					onClick={() => {
						...
						setConfirmationMessage(
							'Are you sure you want to process all pending documents? This may take a long time.'
						);
				...
				<Button
					...
					onClick={() => {
						...
						setConfirmationMessage(
							'This will find and repair documents that have been partially processed or have invalid data. Continue?'
						);
				...
				<Button
					...
					onClick={() => {
						...
						setConfirmationMessage(
							"This will migrate existing documents to use the documentGroup field. Use this if you have existing documents that don't have this field. Continue?"
						);
				...
				<FormControl size='small' style={{ minWidth: 120 }}>
					<InputLabel id='concurrency-limit-label'>
						Concurrency
					</InputLabel>
					<Select
				...
				>
					...
					<ToggleButton
						value='images-only'
						aria-label='process images only'
					>
					...
					<ToggleButton
						value='stop-at-image'
						aria-label='stop at image analysis'
					>
			...
			{isProcessingAll && (
				<div style={{ marginBottom: '1rem' }}>
					<Paper style={{ padding: '1rem', background: '#f9fafb' }}>
						...
						>
							Processing Status: {activeProcessingCount} active
							jobs
						</Typography>
						<Typography
							variant='body2'
						...
						>
							...
							<Typography variant='body2'>
								{processedCount} /{' '}
								{totalToProcess > 0 ? totalToProcess : '?'}{' '}
								documents processed
							</Typography>
							<Button
								variant='contained'
								...
								onClick={() => {
									...
									setConfirmationMessage(
										'Are you sure you want to stop all document processing? This will abort any active processing jobs.'
									);
			...
			>
							...
							Array.from(
								new Set(
									documents.map((doc) => {
										...
										const groupValue =
											docWithType.documentGroup ||
											docWithType.documentType ||
											(doc.id &&
											typeof doc.id === 'string' &&
											doc.id.toUpperCase().includes('RFK')
												? 'rfk'
					...
					<code>
						{
							documents.filter((doc) => {
								...
								if (docWithType.documentGroup)
									return (
										docWithType.documentGroup.toLowerCase() ===
										'jfk'
									);
								if (docWithType.documentType)
									return (
										docWithType.documentType.toLowerCase() ===
										'jfk'
									);
								return !(
									doc.id &&
				...
				<Typography variant='body2'>
					Current Page: <code>{currentPage}</code> of{' '}
				...
				<Typography variant='body2'>
					Start Index:{' '}
					<code>{(currentPage - 1) * itemsPerPage + 1}</code>
				</Typography>
				<Typography variant='body2'>
					End Index:{' '}
					<code>
