...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      try {
        ...
        const documents = await prisma.document.findMany({
          select: {
            ...
            sourceUrl: true  // This contains the frontend ID
          },
          orderBy: {
            createdAt: 'desc'
          }
        ...
        console.log(`Found ${documents.length} documents for ID mapping`);
        
        // Extract frontend IDs from sourceUrl if possible and create a mapping
        const idMap: {[key: string]: string} = {};
        
        documents.forEach((doc: { id: string, sourceUrl: string | null }) => {
          if (doc.sourceUrl) {
            // Try to extract the frontend ID from the URL
            // Typically it's the last part of the path
            const urlParts = doc.sourceUrl.split('/');
        ...
        if (mapCount === 0 && documents.length > 0) {
          console.warn('No ID mappings were extracted despite having documents. Sample URLs:', 
            documents.slice(0, 3).map((d: { sourceUrl: string | null }) => d.sourceUrl));
          ...
          console.log(`Created fallback 1:1 mapping with ${Object.keys(idMap).length} entries`);
        }
        ...
        return NextResponse.json(
          { error: 'Failed to create ID map', details: mapError instanceof Error ? mapError.message : 'Unknown error' },
          { status: 500 }
        );
      }
    }
    ...
    try {
      ...
      console.log(`Fetched ${documents.length} documents (page ${page}, limit ${limit})`);
      ...
      return NextResponse.json(
        { error: 'Failed to list documents', details: listError instanceof Error ? listError.message : 'Unknown error' },
        { status: 500 }
      );
    }
  } catch (error) {
