  ...
  try {
    ...
    switch (type) {
      case 'network':
        ...
        return await getNetworkData(documentId, document?.title || documentId, connectionLayers);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch connection data', details: error instanceof Error ? error.message : undefined },
      { status: 500 }
    );
  } finally {
...
async function getNetworkData(documentId: string, documentTitle: string, layers: number) {
  console.log(`Getting network data for document ID: ${documentId} with ${layers} connection layers`);
  ...
  console.log(`Found ${persons.length} persons, ${places.length} places, and ${objects.length} objects`);
  ...
  if (layers <= 1) {
    console.log('Returning only first layer connections (entities directly connected to source document)');
    return NextResponse.json({
      results: {
        nodes,
        links
  ...
  if (allEntities.length > 0) {
    console.log(`Searching for related documents with the same entities...`);
    ...
    const relatedDocuments = await prisma.document.findMany({
      where: {
        id: { not: documentId },
        OR: [
          ...(persons.length > 0 ? [{ allNames: { hasSome: persons } }] : []),
          ...(places.length > 0 ? [{ allPlaces: { hasSome: places } }] : []),
          ...(objects.length > 0 ? [{ allObjects: { hasSome: objects } }] : [])
        ]
      },
      select: {
        id: true,
    ...
    console.log('Related document IDs:', relatedDocuments.map(doc => doc.id));
    
    // Check for any documents that still have oldId or archiveId values (should be none)
    const docsWithOldId = relatedDocuments.filter(doc => doc.oldId !== null);
    const docsWithArchiveId = relatedDocuments.filter(doc => doc.archiveId !== null);
    
    if (docsWithOldId.length > 0) {
      console.warn(`WARNING: Found ${docsWithOldId.length} documents with non-null oldId values`);
      console.warn('This should not happen if the fields were cleared properly');
    }
    
    if (docsWithArchiveId.length > 0) {
      console.warn(`WARNING: Found ${docsWithArchiveId.length} documents with non-null archiveId values`);
      console.warn('This should not happen if the fields were cleared properly');
    }
    ...
    titleMap.forEach((docIds, title) => {
      if (docIds.length > 1) {
        ...
        console.warn(`Found ${docIds.length} documents with the same title: "${title}"`);
    ...
    relatedDocuments.forEach(doc => {
      ...
      let entityCount = (doc.allNames?.length || 0) + (doc.allPlaces?.length || 0) + (doc.allObjects?.length || 0);
      ...
      for (const otherDoc of relatedDocuments) {
        ...
        if (isRelated) {
          ...
          if (hasDashes && !otherHasDashes) {
            ...
            const toRemove = doc.id < otherDoc.id ? otherDoc.id : doc.id;
    ...
    uniqueDocMap.forEach(docs => {
      if (docs.length > 1) {
        ...
        const dashDoc = docs.find(d => d.id.includes('-'));
    ...
    const dedupedRelatedDocs = relatedDocuments.filter(doc => !duplicateTracker.has(doc.id));
    console.log(`Removed ${relatedDocuments.length - dedupedRelatedDocs.length} duplicate documents`);
    ...
    dedupedRelatedDocs.forEach(relDoc => {
      ...
      const docEntities = [...relatedPersons, ...relatedPlaces, ...relatedObjects];
      ...
      docEntities.forEach(entity1 => {
        if (allEntities.includes(entity1)) {
          docEntities.forEach(entity2 => {
            if (entity1 !== entity2 && allEntities.includes(entity2)) {
    ...
    Object.entries(entityRelationships).forEach(([entity1, relatedEntitiesSet]) => {
      if (allEntityIds[entity1]) {
        relatedEntitiesSet.forEach(entity2 => {
          if (allEntityIds[entity2]) {
            ...
            const linkKey = [entity1Id, entity2Id].sort().join('-');
      ...
      if (secondaryEntities.length > 0) {
        ...
        const tertiaryDocuments = await prisma.document.findMany({
          where: {
            ...
            OR: [
              { allNames: { hasSome: secondaryEntities.filter(e => typeof e === 'string') as string[] } },
              { allPlaces: { hasSome: secondaryEntities.filter(e => typeof e === 'string') as string[] } },
              { allObjects: { hasSome: secondaryEntities.filter(e => typeof e === 'string') as string[] } }
            ]
          },
          select: {
            id: true,
        ...
        console.log(`Found ${tertiaryDocuments.length} tertiary documents for layer 4`);
        ...
        tertiaryDocuments.forEach(tertDoc => {
          ...
          if (!tertiaryDuplicateTracker.has(tertDoc.id)) {
            for (const otherDoc of tertiaryDocuments) {
              ...
              if (
                (tertDoc.oldId && tertDoc.oldId === otherDoc.id) ||
                (otherDoc.oldId && otherDoc.oldId === tertDoc.id) ||
                (tertDoc.archiveId && tertDoc.archiveId === otherDoc.id) ||
                (otherDoc.archiveId && otherDoc.archiveId === tertDoc.id)
              ) {
                ...
                if (tertHasDashes && !otherHasDashes) {
                  ...
                  const toRemove = tertDoc.id < otherDoc.id ? otherDoc.id : tertDoc.id;
        ...
        console.log(`Removed ${tertiaryDocuments.length - dedupedTertiaryDocs.length} duplicate tertiary documents`);
        ...
        dedupedTertiaryDocs.forEach(tertDoc => {
          if (!addedDocumentIds.has(tertDoc.id)) {
            ...
            nodes.forEach(node => {
              if (node.group === 'secondary') {  // Only connect to secondary entities
                if (node.type === 'person' && node.data.name && tertDocPersons.includes(node.data.name)) {
                  links.push({
                    source: tertDoc.id,
                    target: node.id,
                    type: 'mentions_person',
                  });
                } else if (node.type === 'place' && node.data.place && tertDocPlaces.includes(node.data.place)) {
                  links.push({
                    source: tertDoc.id,
                    target: node.id,
                    type: 'mentions_place',
                  });
                } else if (node.type === 'object' && node.data.object && tertDocObjects.includes(node.data.object)) {
                  links.push({
                    source: tertDoc.id,
                    target: node.id,
                    type: 'mentions_object',
            ...
            if (layers >= 5 && nodes.length < 200) {  // Limit to prevent overwhelming visualization
              // Add new people from tertiary documents (limited to 2 per document)
              tertDocPersons.slice(0, 2).forEach(person => {
                if (!nodes.some(n => n.type === 'person' && n.data.name === person)) {
                  const personId = `person-${person.toLowerCase().replace(/\s+/g, '-')}`;
                  nodes.push({
                    id: personId,
                    type: 'person',
              ...
              tertDocPlaces.slice(0, 2).forEach(place => {
                if (!nodes.some(n => n.type === 'place' && n.data.place === place)) {
                  const placeId = `place-${place.toLowerCase().replace(/\s+/g, '-')}`;
                  nodes.push({
                    id: placeId,
                    type: 'place',
  ...
  console.log(`Returning network with ${nodes.length} nodes and ${links.length} links`);
  return NextResponse.json({
    results: {
      nodes,
      links
...
async function getGeoData(documentId: string) {
  ...
  try {
    ...
    if (!document || !document.allPlaces || document.allPlaces.length === 0) {
      return NextResponse.json({
        results: {
          places: []
        }
    ...
    const geoData = Object.keys(placeCounts).map(place => {
      ...
      const hash = place.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
...
async function getDateStatistics() {
  ...
  const serializedStats = dateStats.map((stat: any) => ({
    date: stat.date,
...
async function getPlaceDocuments(placeName: string | null) {
  if (!placeName) {
    ...
    // Convert BigInt to regular numbers before sending in the response
    const serializedStats = placeStats.map((stat: any) => ({
      place: stat.place,
      document_count: Number(stat.document_count)
    }));
...
async function getPersonDocuments(personName: string | null) {
  if (!personName) {
    ...
    // Convert BigInt to regular numbers before sending in the response
    const serializedStats = personStats.map((stat: any) => ({
      person: stat.person,
      document_count: Number(stat.document_count)
    }));
...
async function getObjectDocuments(objectName: string | null) {
  if (!objectName) {
    ...
    // Convert BigInt to regular numbers before sending in the response
    const serializedStats = objectStats.map((stat: any) => ({
      object: stat.object,
      document_count: Number(stat.document_count)
    }));
